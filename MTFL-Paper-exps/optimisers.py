...
class ClientAdam(torch.optim.Adam, ClientOpt):
    ...
    def __init__(   self, params, lr=0.001, betas=(0.9, 0.999), 
        ...
        #for new pytorch versions
        for group in self.param_groups:
            for p in group['params']:
                state = self.state[p]
                if 'step' in state:
                    if not isinstance(state['step'], torch.Tensor):
                        state['step'] = torch.tensor([state['step']], dtype=torch.int)
    ...
    def set_params(self, params):
        ...
        for key in self.state.keys():
            ...
            #i += 3
            if isinstance(params[i], torch.Tensor):
                self.state[key]['step'] = params[i]
            else:
                step_value = params[i].item() if hasattr(params[i], 'item') else params[i]
                self.state[key]['step'] = torch.tensor([step_value], dtype=torch.int)
            self.state[key]['exp_avg'].copy_(torch.tensor(params[i+1]))
            self.state[key]['exp_avg_sq'].copy_(torch.tensor(params[i+2]))
            i += 3
