    ...
    def set_params(self, params):
        ...
        with torch.no_grad():
            ...
            for p in self.parameters():
                # Convert numpy array to tensor and place on correct device
                param_tensor = torch.tensor(params[i], dtype=p.dtype, device=self.device)
            ...
            for bn in self.bn_layers:
                # Set running_mean
                mean_tensor = torch.tensor(params[i], dtype=bn.running_mean.dtype, device=self.device)
                bn.running_mean.copy_(mean_tensor)
                i += 1
                
                # Set running_var
                var_tensor = torch.tensor(params[i], dtype=bn.running_var.dtype, device=self.device)
                bn.running_var.copy_(var_tensor)
                i += 1
